...
def pl_buy(context):
    ...
    for pl_code in g.pl_stock_pool:
        ...
        #上穿
        #pl_short_ma = pd.rolling_mean(pl_close_data,PL_BUY_SHORT_MA)错误用法
        #pl_long_ma  = pd.rolling_mean(pl_close_data,PL_BUY_LONG_MA)错误用法
        pl_short_ma = pl_close_data.rolling(window=PL_BUY_SHORT_MA).mean()
        pl_long_ma  = pl_close_data.rolling(window=PL_BUY_LONG_MA).mean()
        #pl_short_ma = pd.Series.rolling(pl_close_data,window=PL_BUY_SHORT_MA).mean()也可以
        #pl_long_ma  = pd.Series.rolling(pl_close_data,window=PL_BUY_LONG_MA).mean()也可以
...
def pl_sell(context):
    ...
    for pl_code in context.portfolio.positions.keys():
        ...
        #下穿
        #pl_short_ma = pd.rolling_mean(pl_close_data,PL_SELL_SHORT_MA) 错误用法
        #pl_long_ma  = pd.rolling_mean(pl_close_data,PL_SELL_LONG_MA)错误用法
        ...
        #pl_short_ma = pd.Series.rolling(pl_close_data,window=PL_SELL_SHORT_MA).mean()也可以
        #pl_long_ma  = pd.Series.rolling(pl_close_data,window=PL_SELL_LONG_MA).mean()也可以
        if pl_cross(pl_short_ma,pl_long_ma) < 0:
            # 卖出
