...
def pl_after_market_close(context):
    ...
    g.pl_stock_pool_update_day = (
        g.pl_stock_pool_update_day + 1) % PL_CHANGE_STOCK_POOL_DAY_NUMBER
    return
...
def pl_load_fundamentals_data(context, pl_date):
    ...
    for pl_index in range(len(pl_df['code'])):
        ...
        pl_raw_data_item = {
            'code': pl_code, 'market_cap': pl_df['market_cap'][pl_index], 'pe_ratio': pl_df['pe_ratio'][pl_index]}
...
def pl_load_change_pct_data(context, pl_date, pl_codes):
    ...
    # 获取25个交易日的日期
    pl_pre_25_dates = jqdata.get_trade_days(
        start_date=None, end_date=pl_date, count=pl_count)
    pl_pre_25_date = pl_pre_25_dates[0]
    pl_pre_1_date = pl_pre_25_dates[-1]
    for pl_code in pl_codes:
        pl_pre_25_data = get_price(pl_code, start_date=None, end_date=pl_pre_25_date, frequency='daily', fields=[
                                   'close'], skip_paused=True, fq='post', count=1)
        pl_pre_1_data = get_price(pl_code, start_date=None, end_date=pl_pre_1_date, frequency='daily', fields=[
                                  'close'], skip_paused=True, fq='post', count=1)
        ...
        if pl_pre_25_close != None and pl_pre_1_close != None and not math.isnan(pl_pre_25_close) and not math.isnan(pl_pre_1_close):
            pl_change_pct = (pl_pre_1_close -
                             pl_pre_25_close) / pl_pre_25_close
...
def pl_stock_pool(context):
    ...
    # 按照财务信息中的总市值降序排序
    pl_raw_data = sorted(
        pl_raw_data, key=lambda item: item['market_cap'], reverse=True)
    # 剔除总市值排名最小的10%的股票
    ...
    pl_change_pct_dict_list = pl_load_change_pct_data(
        context, pl_date, pl_filtered_pe)
    # 按照涨跌幅升序排序
    pl_change_pct_dict_list = sorted(
        pl_change_pct_dict_list, key=lambda item: item['change_pct'], reverse=False)
    # 取跌幅前10%的股票
    pl_change_pct_dict_list = pl_change_pct_dict_list[0:(
        int(len(pl_change_pct_dict_list)*0.1))]
...
def pl_buy(context):
    ...
    for pl_code in g.pl_stock_pool:
        ...
        pl_close_data = attribute_history(security=pl_code, count=pl_count, unit=PL_UNIT, fields=[
                                          'close'], skip_paused=True, df=True, fq='pre')['close']
        if (list(np.isnan(pl_close_data)).count(True) > 0) or (len(list(pl_close_data)) < pl_count):
            continue

        # MA3上穿MA200
        # pl_short_ma = pd.rolling_mean(pl_close_data,PL_BUY_SHORT_MA)错误
        # pl_long_ma  = pd.rolling_mean(pl_close_data,PL_BUY_LONG_MA)错误
        pl_short_ma = pl_close_data.rolling(window=PL_BUY_SHORT_MA).mean()
        pl_long_ma = pl_close_data.rolling(window=PL_BUY_LONG_MA).mean()
        ...
        if pl_cross(pl_short_ma, pl_long_ma) > 0:
            ...
            if (pl_order_ is not None) and (pl_order_.filled > 0):
                log.info("交易 买入", pl_code, "预期买入",
                         pl_order_.amount, "实际买入", pl_order_.filled)
    return
...
def pl_sell(context):
    ...
    for pl_code in context.portfolio.positions.keys():
        ...
        # MA3下穿MA200
        # pl_short_ma = pd.rolling_mean(pl_close_data,PL_SELL_SHORT_MA)错误
        # pl_long_ma  = pd.rolling_mean(pl_close_data,PL_SELL_LONG_MA)错误
        pl_short_ma = pl_close_data.rolling(window=PL_SELL_SHORT_MA).mean()
        pl_long_ma = pl_close_data.rolling(window=PL_SELL_LONG_MA).mean()

        if pl_cross(pl_short_ma, pl_long_ma) < 0:
            # 卖出股票
            pl_order_ = order_target(security=pl_code, amount=0)
            if (pl_order_ is not None) and (pl_order_.filled > 0):
                log.info("交易 卖出", pl_code, "卖出数量", pl_order_.filled,
                         "剩余数量", (pl_order_.amount - pl_order_.filled))
    return
